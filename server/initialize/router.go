...
func Routers() *gin.Engine {
	...
	PublicGroup := Router.Group("")
	{
		...
		router.InitWebTerminalRouter(PublicGroup)
	}
	...
	PrivateGroup.Use(middleware.JWTAuth()).Use(middleware.CasbinHandler())
	{
		...
		router.InitDnsUserRouter(PrivateGroup)    // DNS用户管理
		router.InitDomainListRouter(PrivateGroup) // 域名管理
		router.InitRecordListRouter(PrivateGroup) // 域名解析管理
		router.InitSSLCheckRouter(PrivateGroup)
		router.InitSSHUserRouter(PrivateGroup)
		router.InitHostInfoRouter(PrivateGroup)
		router.InitCronJobRouter(PrivateGroup)
		// router.InitWebTerminalRouter(PrivateGroup)	//TODO: 需要前端ws传token过来
		//router.InitSSLCheckRouter(PrivateGroup)				// 域名证书检查
